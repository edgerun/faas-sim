...
def get_experiment_pair(rps: int, duration: int, topo_type: RealTopoType, lb_target_fraction: float, osmotic_pressure_threshold: float, osmotic_hysteresis: float) -> List[Experiment]:
    osmotic = Experiment(f'{rps}rps_{duration}s_{topo_type_to_printable_string(topo_type)}_osmotic',
                 ...
                 lb_scaler_factory=OsmoticLoadBalancerScalerFactory(pressure_threshold=osmotic_pressure_threshold, hysteresis=osmotic_hysteresis),
...
def get_parametrization_set(rps=25, duration=2000, topo_type=RealTopoType.CITY, lb_target_fraction=0.2, osmotic_pressure_threshold=0.025, osmotic_hysteresis=0.03):
...
f = open('/home/jp/Documents/tmp/osmotic_basic_fixed_fx_scaling.dump', 'wb')
