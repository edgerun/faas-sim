...
from ext.jjnp21.schedulers.predicates import NoLoadBalancerRunningYet, OsmoticTargetPredicate
...
class OsmoticLoadBalancerScheduler(LoadBalancerScheduler):
    ...
    def schedule(self, pod: Pod) -> SchedulingResult:
        ...
        if suggested_host is not None:
            ...
            logger.info("selected node %s for pod %s from total of %d nodes", suggested_host.name, pod.name,
                        len(filtered))
    ...
    def create(env: Environment, predicates: List[Predicate]) -> LoadBalancerScheduler:
        ...
        changed_predicates.append(OsmoticTargetPredicate())
        return OsmoticLoadBalancerScheduler(env.cluster, changed_predicates)
