...
class PowerPredictionBenchmark(Benchmark):
    ...
    def setup(self, env: Environment):
        ...
        containers.put(ImageProperties('resi5/resnet-inference-cpu', parse_size_string('2.7G'), arch='x86'))
        containers.put(ImageProperties('resi5/resnet-inference-cpu', parse_size_string('1G'), arch='aarch64'))

        containers.put(ImageProperties('resi5/resnet-inference-gpu', parse_size_string('2.7G'), arch='x86'))
        containers.put(ImageProperties('resi5/resnet-inference-gpu', parse_size_string('1G'), arch='aarch64'))

        containers.put(ImageProperties('resi5/efficientnet-inference-cpu', parse_size_string('2.6G'), arch='x86'))
        containers.put(ImageProperties('resi5/efficientnet-inference-cpu', parse_size_string('978M'), arch='aarch64'))

        containers.put(ImageProperties('resi5/efficientnet-inference-gpu', parse_size_string('2.6G'), arch='x86'))
        containers.put(ImageProperties('resi5/efficientnet-inference-gpu', parse_size_string('978M'), arch='aarch64'))

        containers.put(ImageProperties('edgerun/objectdetection-cpu', parse_size_string('970M'), arch='x86'))
        containers.put(ImageProperties('edgerun/objectdetection-cpu', parse_size_string('828M'), arch='aarch64'))
    ...
    def prepare_deployments(self, env: Environment) -> List[FunctionDeployment]:
        ...
        for i in range(n):
            function_deployment = self.prepare_deployment(env, f'resnet-inference-{i}', 'resi5/resnet-inference-gpu', i)
    ...
    def prepare_resnet_inference_cpu_deployment(self, env: Environment):
        ...
        resnet_inference_fn = Function(resnet_inference, fn_images=[resnet_inference_cpu])
        ...
        resnet_inference_fn_container = FunctionContainer(resnet_inference_cpu,
                                                          labels={'workers': '1',
