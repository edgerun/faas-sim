...
def main():
    ...
    for k, df in dfs.items():
        df.to_csv(f'data/output/{nodename}/efficientnet-inference-gpu/{k}.csv', index=False)
...
class PowerPredictionBenchmark(Benchmark):
    ...
    def setup(self, env: Environment):
        ...
        containers.put(ImageProperties('resi5/efficientnet-inference-gpu', parse_size_string('2640M'), arch='x86'))
    ...
    def prepare_deployments(self, env: Environment) -> List[FunctionDeployment]:
        ...
        for i in range(n):
            function_deployment = self.prepare_deployment(env, f'efficientnet-inference-{i}', 'resi5/efficientnet-inference-gpu', i)
